...
#define CONVERTERMODE
#ifdef CONVERTERMODE
void ConvertOneXtoDBO (LPSTR pFileFolderName)
{
...
void scanallfolder (std::string subThisFolder_s, std::string subFolder_s)
{
	...
	for (int c = 1; c <= iFoldersCount; c++)
	{
		...
		if (strcmp (pFileFolderName, ".") != NULL && strcmp (pFileFolderName, "..") != NULL)
		{
			...
			{
				// look for an X file with no DBO counterpart
				ConvertOneXtoDBO(pFileFolderName);
				/*
				if (strnicmp(pFileFolderName + strlen(pFileFolderName) - 2, ".x", 2) == NULL)
				{
					// is there a DBO already?
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// LB: IMPORTANT FIX - ensures a single UV layer, allowing old DBOs to rein!!
	SetLegacyModeOn();

	// single or batch convert
	if (gRefCommandLineString && strlen(gRefCommandLineString) > 0)
	{
		ConvertOneXtoDBO(gRefCommandLineString);
	}
	else
	{
		...
		if (g_iCountConversions > 0)
		{
			...
			if (g_iCountConversions == 1)
				sprintf(pHelpfulPrompt, "Successfully converted 1 model from X file to DBO", g_iCountConversions);
			else
				sprintf(pHelpfulPrompt, "Successfully converted %d models from X file to DBO", g_iCountConversions);
