...
extern DBPRO_GLOBAL HWND					GlobalHwndCopy;
extern DBPRO_GLOBAL char					m_pWorkString[_MAX_PATH];

DARKSDK void MemblocksConstructor ( void )
{
...
DARKSDK void MemblocksDestructor ( void )
{
...
DARKSDK void MemblocksSetErrorHandler ( LPVOID pErrorHandlerPtr )
{
	// Update error handler pointer
	g_pErrorHandler = (CRuntimeErrorHandler*)pErrorHandlerPtr;
}

DARKSDK void MemblocksPassCoreData( LPVOID pGlobPtr )
{
	...
	/*#ifndef DARKSDK_COMPILE
		if(g_pGlob->g_Bitmap)
		{
			GetBitmapData = ( RetVoidGetBitmapDataPFN ) GetProcAddress ( g_pGlob->g_Bitmap, "?GetBitmapData@@YAXHPAK00PAPAD0_N@Z" );
			SetBitmapData = ( RetVoidSetBitmapDataPFN ) GetProcAddress ( g_pGlob->g_Bitmap, "?SetBitmapData@@YAXHKKKPADK@Z" );
		}
		if(g_pGlob->g_Image)
		{
			GetImageData = ( RetVoidGetBitmapDataPFN ) GetProcAddress ( g_pGlob->g_Image, "?GetImageData@@YAXHPAK00PAPAD0_N@Z" );
			SetImageData = ( RetVoidSetBitmapDataPFN ) GetProcAddress ( g_pGlob->g_Image, "?SetImageData@@YAXHKKKPADK@Z" );
		}
		if(g_pGlob->g_Sound)
		{
			// mike - 300305 - updated function pointer
			GetSoundData = ( RetVoidGetSoundDataPFN ) GetProcAddress ( g_pGlob->g_Sound, "?GetSoundData@@YAXHPAK00PAPAD0_NPAUtWAVEFORMATEX@@@Z" );
			SetSoundData = ( RetVoidSetSoundDataPFN ) GetProcAddress ( g_pGlob->g_Sound, "?SetSoundData@@YAXHKKKPADKUtWAVEFORMATEX@@@Z" );
		}
		if(g_pGlob->g_Basic3D)
		{
			GetMeshData = ( RetVoidGetMeshDataPFN ) GetProcAddress ( g_pGlob->g_Basic3D, "?GetMeshData@@YAXHPAK00PAPAD0_N@Z" );
			SetMeshData = ( RetVoidSetMeshDataPFN ) GetProcAddress ( g_pGlob->g_Basic3D, "?SetMeshData@@YAXHKKPADK@Z" );
		}
	#else
		GetBitmapData = dbGetBitmapData;
		SetBitmapData = dbSetBitmapData;
		
		GetImageData = dbGetImageData;
		SetImageData = dbSetImageData;
		
		GetSoundData = dbGetSoundData;
		SetSoundData = dbSetSoundData;
		
		GetMeshData = dbGetMeshData;
		SetMeshData = dbSetMeshData;
	#endif*/
	
}

DARKSDK void MemblocksRefreshGRAFIX ( int iMode )
{
}
...
DBPRO_GLOBAL LPSTR MBGetReturnStringFromWorkString(void)
{
...
DARKSDK BOOL DB_MemblockFromBitmap(int mbi, int bitmapindex)
{
	...
	// Get bitmap data
	GetBitmapData ( bitmapindex, &dwWidth, &dwHeight, &dwDepth, &pData, &dwDataSize, true );
	if(bitmapindex==0 || pData)
	{
		...
		// Free bitmap data
		GetBitmapData ( 0, 0, 0, 0, &pData, 0, false );
	}
	else
	{
...
DARKSDK BOOL DB_MemblockFromImage(int mbi, int imageindex)
{
	...
	// Get bitmap data
	GetImageData ( imageindex, &dwWidth, &dwHeight, &dwDepth, &pData, &dwDataSize, true );
	if(pData)
	{
		// lee - 220206 - u60 - Free any old memblock
		if(gpMemblock[mbi])
		...
		// Free bitmap data
		GetImageData ( 0, 0, 0, 0, &pData, 0, false );
	}
	else
	{
...
DARKSDK BOOL DB_ImageFromMemblock(int mbi, int imageindex)
{
	...
	if(gpMemblock[mbi])
	{
		...
		SetImageData ( imageindex, dwWidth, dwHeight, dwDepth, pData, dwDataSize );
	}
	else
	{
		RunTimeError(RUNTIMEERROR_MEMBLOCKNOTEXIST);
...
DARKSDK BOOL DB_MemblockFromSound(int mbi, int soundindex)
{
	...
	GetSoundData ( soundindex, &dwBitsPerSecond, &Frequency, &Duration, &pData, &dwDataSize, true, &wfx );
	if(soundindex==0 || pData)
	{
		...
		// Free sound data
		GetSoundData ( 0, 0, 0, 0, &pData, 0, false, 0 );
	}
	else
	#endif
	{
...
DARKSDK BOOL DB_SoundFromMemblock(int mbi, int soundindex)
{
	...
	if(gpMemblock[mbi])
	{
		...
		#ifndef NOSTEAMORVIDEO
		SetSoundData ( soundindex, dwBitsPerSecond, Frequency, Duration, pData, dwDataSize, wfx );
		#endif
	}
	else
	{
...
DARKSDK BOOL DB_MemblockFromMesh(int mbi, int meshid)
{
	...
	// Get Mesh data
	GetMeshData ( meshid, &dwFVF, &dwFVFSize, &dwVertMax, &pData, &dwDataSize, true );
	if(meshid==0 || pData)
	{
		// lee - 220206 - u60 - Free any old memblock
		if(gpMemblock[mbi])
		...
		// Free mesh data
		GetMeshData ( 0, 0, 0, 0, &pData, 0, false );
	}
	else
	{
...
DARKSDK BOOL DB_MeshFromMemblock(int mbi, int meshid)
{
	...
	if(gpMemblock[mbi])
	{
		...
		// Recreate from memblock data
		SetMeshData ( meshid, dwFVF, dwFVFSize, pData, dwVertMax );
	}
	else
	{
		RunTimeError(RUNTIMEERROR_MEMBLOCKNOTEXIST);
...
DARKSDK float ReadMemblockFloat( int mbi, int pos )
{
	float data=0.0f;
	if(mbi>=1 && mbi<=257)
	{
...
void RefreshGRAFIXMemblocks ( int iMode )
{
	RefreshGRAFIX ( iMode );
}
